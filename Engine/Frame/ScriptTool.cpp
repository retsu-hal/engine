...
{
	// プロジェクトのスクリプト → エンジンのコンポーネントの順に探す
	std::string engine = EnginePaths::GetEngineDir();
	const std::string folders[] = { "Script", "Scene", engine + "\\Component", engine + "\\Frame" };
	for (const std::string& folder : folders)
	{
		std::string path = folder + "\\" + typeName + ".cpp";
