...
// カメラだけの空のシーン（新しいシーン・最初のシーンがないとき）
constexpr const char* EMPTY_SCENE_JSON =
	"{\"version\":1,\"objects\":[{\"id\":1,\"class\":\"GameObject\",\"name\":\"Main Camera\",\"parent\":0,\"layer\":1,"
	"\"tag\":\"MainCamera\",\"position\":[0,1,-10],\"rotation\":[0,0,0],\"scale\":[1,1,1],\"components\":[{\"type\":\"CameraComponent\",\"enabled\":true}]}]}";
...
public:
	...
	// REGISTER_SCENE したシーンを名前で切り替える（見つからなければ false）
	static bool ChangeSceneByName(const std::string& sceneClassName);

	// ProjectSettings の StartScene を読み込む（起動時）
