...
private:
	...
	static bool        m_SceneVisible;
	static bool        m_GameHovered;
	static bool        m_GameVisible;
	static unsigned int m_DockSpaceID;
	static bool        m_ResetLayout;
	static int         m_FocusWindow;			// 0:なし 1:Scene 2:Game（再生・停止で切り替える）

	// 各ウィンドウを表示するか（Window メニュー）
	// ※ DLL から出すクラスでは「static bool a, b;」のように1行にまとめると C2487 になるので1つずつ書く
	static bool        m_ShowScene;
	static bool        m_ShowGame;
	static bool        m_ShowHierarchy;
	static bool        m_ShowInspector;
	static bool        m_ShowProject;
...
public:
	...
	static bool UseEditorCamera() { return true; }	// Scene ビューはいつもエディタカメラ（ゲームのカメラは Game ビュー）
	static void RequestStep(int frames = 1) { m_StepFrames += frames; }

	// シーンビュー
	static bool        IsSceneViewHovered() { return m_SceneHovered; }
	static bool        IsSceneViewVisible() { return m_SceneVisible; }

	// Game ビュー
	static bool        IsGameViewHovered() { return m_GameHovered; }
	static bool        IsGameViewVisible() { return m_GameVisible; }

	// ドッキング（初回に Unity 風の配置を作る）
	static void        SetDockSpaceID(unsigned int id) { m_DockSpaceID = id; }
