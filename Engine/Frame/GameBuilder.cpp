...
void GameBuilder::Start(bool runAfterBuild)
{
	...
	// エンジンのソリューション（Engine.dll・Player.exe・GameScripts.dll をまとめてビルドする）
	std::string solutionDir = EnginePaths::GetSolutionDir();
	std::string solution = solutionDir + "\\GM31Engine.sln";
	std::wstring msbuild = FindMSBuild();
	if (solutionDir.empty() || msbuild.empty())
	{
		Debug::LogError("ビルドできません: %s", solutionDir.empty() ? "GM31Engine.sln が見つかりません" : "MSBuild が見つかりません（Visual Studio が必要です）");
	...
	std::wstring command = L"\"" + msbuild + L"\" \"" + Utf8ToWide(solution) +
		L"\" /p:Configuration=Game /p:Platform=x64 /m /nologo /v:minimal";
...
{
	// Game 構成の出力（bin\\Game）に Player.exe・Engine.dll・GameScripts.dll・assimp の DLL がある
	std::string binDir = EnginePaths::GetSolutionDir() + "\\bin\\Game";
	std::string exe = binDir + "\\Player.exe";
	...
	// DLL（Engine / GameScripts / assimp）
	WIN32_FIND_DATAA find;
	HANDLE handle = FindFirstFileA((binDir + "\\*.dll").c_str(), &find);
	if (handle != INVALID_HANDLE_VALUE)
	{
		do { CopyFileA((binDir + "\\" + find.cFileName).c_str(), (s_OutputFolder + "\\" + find.cFileName).c_str(), FALSE); } while (FindNextFileA(handle, &find));
		FindClose(handle);
	}

	// シェーダー（Engine\\shader）
	std::string shaderDir = EnginePaths::GetEngineDir() + "\\shader";
	handle = FindFirstFileA((shaderDir + "\\*.cso").c_str(), &find);
	if (handle != INVALID_HANDLE_VALUE)
	{
		do { CopyFileA((shaderDir + "\\" + find.cFileName).c_str(), (s_OutputFolder + "\\shader\\" + find.cFileName).c_str(), FALSE); } while (FindNextFileA(handle, &find));
		FindClose(handle);
	}
