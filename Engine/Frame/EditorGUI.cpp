...
std::string  EditorGUI::m_ScenePath;	// 起動時に Manager::LoadStartScene が決める
...
GameObject* EditorGUI::CreateObject(int type)
{
	...
	switch (type)
	{
	case 0: object = Primitives::CreateEmpty(position);   break;
	case 1: object = Primitives::CreateCube(position);    break;
	case 2: object = Primitives::CreateSphere(position);  break;
	case 3: object = Primitives::CreateCapsule(position); break;
	case 4:
		object = Primitives::CreateCamera(position);
...
void EditorGUI::NewScene()
{
	...
	UndoSystem::Clear();
	Manager::LoadSceneText(EMPTY_SCENE_JSON);
}
